...
{
    [Export(typeof(IAuthorizationServerHost))]
    internal class OAuth2AuthorizationServer : IAuthorizationServerHost
    {
        ...
        public AccessTokenResult CreateAccessToken(IAccessTokenRequest accessTokenRequestMessage)
        {
            ...
            accessToken.Lifetime = TimeSpan.FromMinutes(30);
        ...
        public IClientDescription GetClient(string clientIdentifier)
        {
            ...
            return new ClientDescriptionAdapter(consumerRow);
        }
        ...
        public bool CanBeAutoApproved(EndUserAuthorizationRequest authorizationRequest)
        {
            ...
            if (authorizationRequest.ResponseType == EndUserAuthorizationResponseType.AuthorizationCode)
            {
                ...
                var requestingClient = DataManager.ClientRepository.Get(c => c.ClientIdentifier == authorizationRequest.ClientIdentifier).First();
        ...
        private bool IsAuthorizationValid(HashSet<string> requestedScopes, string clientIdentifier, DateTime issuedUtc, string username)
        {
            ...
            var grantedScopeStrings = from auth in DataManager.ClientAuthorizationRepository.Get()
                                      where
                                          auth.OAuth_Client.ClientIdentifier == clientIdentifier &&
                                          auth.CreatedOnUtc <= issuedUtc &&
                                          (!auth.ExpirationDateUtc.HasValue || auth.ExpirationDateUtc.Value >= DateTime.UtcNow) &&
                                          auth.OAuth_Users.OpenIDClaimedIdentifier == username
                                      select auth.Scope;
